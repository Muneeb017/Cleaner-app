package com.muneeb.cleaner.plus.ui.whitelist
...
import com.muneeb.cleaner.plus.MainActivity
import com.muneeb.cleaner.plus.R
import com.muneeb.cleaner.plus.databinding.ActivityWhitelistBinding
...
class WhitelistActivity : AppCompatActivity() {
        ...
        val mDialog = MaterialDialog.Builder(this)
            ...
            .setPositiveButton(getString(R.string.clean)) { dialogInterface: DialogInterface, _: Int ->
                ...
                MainActivity.preferences?.edit()
                    ?.putStringSet(getString(R.string.key_whitelist), HashSet(whiteList))?.apply()
            ...
            if (result.resultCode == RESULT_OK) {
                ...
                if (uri != null) {
                    ...
                    if (path != null && !whiteList.contains(path)) {
                        whiteList.add(path)
                        MainActivity.preferences?.edit()
                            ?.putStringSet(getString(R.string.key_whitelist), HashSet(whiteList))
                            ?.apply()
                        loadViews()
                    }
                }
    ...
    companion object {
        ...
        fun getWhiteList(preferences: SharedPreferences?): List<String?> {
            if (whiteList.isEmpty()) {
                if (preferences != null) {
                    whiteList = (preferences.getStringSet("whitelist", emptySet())?.toList()
                        ?.toMutableList() ?: ArrayList()) as ArrayList<String>
                }
                whiteList.remove("[")
                whiteList.remove("]")
            }

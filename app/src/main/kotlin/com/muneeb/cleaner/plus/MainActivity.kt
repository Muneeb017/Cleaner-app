package com.muneeb.cleaner.plus
...
import com.muneeb.cleaner.plus.databinding.ActivityMainBinding
import com.muneeb.cleaner.plus.ui.about.AboutActivity
import com.muneeb.cleaner.plus.ui.imageoptimizer.ImageOptimizerActivity
import com.muneeb.cleaner.plus.ui.settings.SettingsActivity
import com.muneeb.cleaner.plus.ui.startup.StartupActivity
import com.muneeb.cleaner.plus.ui.whitelist.WhitelistActivity
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.navigationView.setNavigationItemSelectedListener { MenuItem: MenuItem ->
            when (MenuItem.itemId) {
                R.id.nav_drawer_settings -> startActivity(
                    Intent(
                        this,
                        SettingsActivity::class.java
                    )
                )
                R.id.nav_drawer_whitelist -> startActivity(
                    Intent(
                        this,
                        WhitelistActivity::class.java
                    )
                )
                R.id.nav_image_optimizer -> startActivity(
                    Intent(
                        this,
                        ImageOptimizerActivity::class.java
                    )
                )
    ...
    private fun clean() {
        requestStoragePermissions()
        if (!FileScanner.isRunning) {
            val oneClickCleanEnabled =
                preferences.getBoolean(getString(R.string.key_one_click_clean), false)
    ...
    private fun scan(delete: Boolean) {
        ...
        val fileScanner = FileScanner(path, this)
            ...
            .setInvalid(
                preferences.getBoolean(
                    getString(R.string.key_invalid_media_cleaner),
                    false
                )
            )
            .setDelete(delete)
        ...
        runOnUiThread {
            if (delete) binding.textViewStatus.text =
                getString(R.string.freed) + " " + convertSize(kilobytesTotal) else binding.textViewStatus.text =
